...
function maquinasDisponiveis() {
    ...
    const maquinasAtivas = ["172.16.100.1", "172.16.100.3"] // arrumar alguma forma de puxar quais maquinas do docker estao ativas

    const maquinasInativas = maquinasEscaneadas.filter(maquina => !maquinasAtivas.includes(maquina)); // realiza a operação : (maquinasEscaneadas - maquinasAtivas), ou seja filtra as maquinas desativadas

    // ------------ Parte responsavel por limpar as maquinas que cairam

    if (maquinasInativas.includes(coordenador)) { // verifica se o coordenador atual caiu e, caso tenha caido, atribui null ao coordenador
        coordenador = null
    }

    if (filaDeEspera.some(elemento => maquinasInativas.includes(elemento))) { // verifica se possui alguma maquina que caiu dentro da fila de espera
        maquinasInativas.forEach(elemento => { // remove todas as maquinas inativas da fila de solicitantes caso haja alguma
            removeDaFilaDeSolicitantes(elemento);
        });
    }

    // ------------------------------------------------------------------

    // ------------ Parte resposavel por atualizar a lista de arrIpsMaquinas com os IPs que estao disponiveis

    const idsDisponiveis = Array.from({ length: maquinasAtivas.length }, (_, index) => index + 1);

    arrIpsMaquinas = maquinasAtivas.map((ip, index) => {
        const idEscolhido = idsDisponiveis[index];
        return { ID: idEscolhido, IP: ip };
    });
...
function criaSolicitante() {
    ...
    maquinasDisponiveis(); // sempre que uma nova solicitação for criada, atualiza o arrIpsMaquinas com as maquinas disponiveis 

    if (!coord) {
        escolheCoordenadorAnel();
    }
